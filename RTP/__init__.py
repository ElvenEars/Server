...
class RTP(object):
    ...
    def __init__(self, RtpSocket = ServerSocket("",0,False), transmitList = {}, slot = "2", group = "100"):
        self._data = b''
        ...
        self._rtp_resp_start = b"\x90\x08\x00\x00\x61\x38\xa8\xbd\x00\x00\x00\x00\xe0\x00\x00\x08"
        #print(bytes.fromhex(self.group)) bytes.fromhex(hex(int(self.group))[2:])
        ...
        self._rtp_resp3 = b"\x90\x08\x00\x00\x61\x15\x0d\xa3\x00\x00\x00\x00\xe0\x00\x00\x08\x00\x00\x00"+bytes.fromhex(hex(int(self.group))[2:])+b"\x00\xff\xfa\xe0\x00\x00\x03\xe8\x10\xf0\x04\x00\x00\xff\x0f\x00\x00\x00\x00\x00\x13\x00\x00\x04\x00\x00\x00\x00"
